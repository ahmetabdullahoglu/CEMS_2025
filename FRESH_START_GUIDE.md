...
**يجب حذف ملف `.env` القديم قبل البدء!**

السبب: ملف `.env.example` القديم كان يحتوي على `localhost` بدلاً من `postgres`. إذا لم تحذف `.env` القديم، ستحصل على خطأ اتصال قاعدة البيانات:
```
OSError: Multiple exceptions: [Errno 111] Connect call failed ('127.0.0.1', 5432)
```

**الحل:** احذف `.env` القديم قبل تشغيل `make setup`، ليتم إنشاؤه من `.env.example` المحدث.
...
# 4. حذف ملف .env القديم (مهم جداً!)
rm -f .env

# 5. حذف ملفات uploads و logs
rm -rf uploads/* logs/*

# 6. حذف قاعدة البيانات المحلية (إن وجدت)
...
make reset-all
docker image rm $(docker images 'cems*' -q) 2>/dev/null || true
rm -f .env  # حذف .env القديم - مهم جداً!
```
